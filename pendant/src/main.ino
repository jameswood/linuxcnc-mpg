...
#define TFT_SCK 36
#define TFT_MOSI 35
#define TFT_MISO -1 // unused by screen
#define TFT_RST 38
...
int col_pins[MATRIX_COLS] = {10, 8, 6, 4, 2, 1};
int row_pins[MATRIX_ROWS] = {7, 9, 5, 11};
...
void display_clear() {
    ...
    tft.drawRect(1, 1, 158, 6 * PSTEP + 5 + 2, WHITE);
    tft.drawRect(2, 2, 156, 6 * PSTEP + 5, LIGHTGREY);

    tft.setTextSize(TEXT_L);
...
void setup() {
    ...
    // fspi.begin(TFT_SCK, TFT_MISO, TFT_MOSI, TFT_CS);
...
void matrix_read() {
    ...
    pinMode(row_pins[MATRIX_ROWS], OUTPUT);
    digitalWrite(row_pins[MATRIX_ROWS], 1);
    ...
    if ((rx_data.values.stats & (1<<0)) == 0) {
        ...
        for (n = 0; n < AXES; n++) {
            if (last_values.leds != rx_data.values.leds || last_values.pos[n] != rx_data.values.pos[n]) {
                ...
                sprintf(tmp_str, "%05.2f", rx_data.values.pos[n]);
                tft.print(tmp_str);
            }
